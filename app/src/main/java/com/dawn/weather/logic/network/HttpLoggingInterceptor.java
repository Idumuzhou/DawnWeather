...
/**
 *  @author: LXL
 *  @description: Http请求拦截器 会拦截请求以及返回数据等信息
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logHeaders) {
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                if (contentLength != 0) {
                    logInfo.append("\n");
                    logInfo.append(json(buffer.clone().readString(charset)));
                }
    ...
    public String json(String json) {
        ...
        try {
            ...
            if (json.startsWith("{")) {
                ...
                message = jsonObject.toString(2).replaceAll("\\\\/", "/");
                return message;
            }
            if (json.startsWith("[")) {
                JSONArray jsonArray = new JSONArray(json);
                message = jsonArray.toString(2).replaceAll("\\\\/", "/");
                return message;
            }
